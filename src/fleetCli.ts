...
async function main(): Promise<void> {
  ...
  if (command === "backup") {
    ...
    const manifestPath = optionalStringFlag(flags, "manifest") ?? path.join(managerRepoPath, "fleet.json");
...
Notes:
  - export writes sanitized Telegram/Claude binding state to JSON.
  - backup uses <manager-repo>/fleet.json when it exists; otherwise it snapshots all live bindings.
  - restore clones missing repos only with --clone and creates Telegram topics only with --create-topics.
  - claudeThreadId is exported as soft state; repo-owned docs/agent context is the durable recovery path.
`);
}
