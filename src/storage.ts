...
function mapBinding(row: BindingRow): TopicBinding {
  return {
    ...
    claudeThreadId: row.claude_thread_id,
    model: row.model,
...
export class Storage {
  ...
  updateBindingModel(bindingId: number, model: string | null): void {
    this.db
      .prepare("UPDATE topic_bindings SET model = ?, claude_thread_id = NULL, updated_at = ? WHERE id = ?")
      .run(model, now(), bindingId);
  }

  updateBindingPlanMode(bindingId: number, planMode: boolean): void {
    this.db
      .prepare("UPDATE topic_bindings SET plan_mode = ?, claude_thread_id = NULL, updated_at = ? WHERE id = ?")
      .run(planMode ? 1 : 0, now(), bindingId);
  }

  updateBindingThread(bindingId: number, claudeThreadId: string | null): void {
    this.db
      .prepare("UPDATE topic_bindings SET claude_thread_id = ?, updated_at = ? WHERE id = ?")
      .run(claudeThreadId, now(), bindingId);
  }
