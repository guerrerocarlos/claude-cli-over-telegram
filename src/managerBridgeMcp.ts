...
async function callTool(params: any): Promise<unknown> {
  ...
  if (!bridgeConfig) {
    throw new Error("Telegram bridge is not configured for this Claude run.");
  }
...
function resolveBridgeConfig(): BridgeConfig | null {
  const env = { ...readEnvFile("/etc/claude-cli-over-telegram/env"), ...process.env };
  const url = env.MANAGER_BRIDGE_URL ?? defaultBridgeUrl(env);
  const token = env.MANAGER_BRIDGE_TOKEN;
...
function inferChatId(env: NodeJS.ProcessEnv): number | null {
  ...
  const databasePath = path.isAbsolute(configuredDatabasePath)
    ...
    : path.resolve("/home/gnu/claude-cli-over-telegram", configuredDatabasePath);
