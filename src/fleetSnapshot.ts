...
export async function exportFleetSnapshot(options: ExportOptions): Promise<void> {
  ...
  try {
    ...
    const snapshot = {
      ...
      notes: [
        "claudeThreadId is soft state. Restore should prefer repo-owned docs/agent context when a thread cannot resume.",
...
export async function backupFleetState(options: {
  ...
  await exportFleetSnapshot({
    databasePath: options.databasePath,
    ...(options.manifestPath && existsSync(options.manifestPath) ? { manifestPath: options.manifestPath } : {}),
    outPath: latestPath,
...
function formatBinding(binding: BindingRow): Record<string, unknown> {
  return {
    ...
    claudeThreadId: binding.claude_thread_id,
    model: binding.model,
